...
export const NATIVE_OKF_SYNTHESIS_OUTLINE_INSTRUCTION = `Text-only evidence-supported synthesis outline target: approximately 120 to 220 words; never exceed 280 words. Begin with one direct framing sentence, then give three to five concise design statements with explicit current-turn citations. Add at most one short privacy or limitation paragraph when material. State clearly that the result is a design proposal rather than a validated theory. Do not mention internal proposal-plan generation or validation. Do not add a long introduction, repeat a conclusion, enumerate every retrieved source, or produce a textual flowchart, ASCII, Mermaid, DOT, Graphviz, PlantUML, or JSON diagram payload.`;
