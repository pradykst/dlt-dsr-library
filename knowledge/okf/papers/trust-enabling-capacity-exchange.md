...
* **Output knowledge.** 19 meta-requirements (Fig. 3), six design principles for establishing inter-organizational trust (tender signaling, identity signaling, authority and fairness, incentives, screening, reputation), 45 figure-explicit MR-to-cooperation-design connectors represented as 53 MR-to-DP edges after expanding the shared DP1/DP2 Signaling target, and 14 design features realizing the principles in a Solidity-based instantiation (Fig. 6).
...
Fig. 3 explicitly states that the meta-requirements relate to one or more design principles and draws 45 vector connectors from individual MR rows to five cooperation-design targets. Authority and fairness, Incentive mechanism, Screening, and Reputation mechanism each identify one principle (DP3-DP6). The Signaling target explicitly contains both DP1 and DP2, so each of its eight incoming connectors is expanded to both member principles in the canonical graph. This produces 53 MR-to-DP `Addresses` edges without using row proximity or thematic similarity. Fig. 6 separately groups each design feature under one governing design principle, producing 14 `Implements` edges.
...
[3] Source evidence: Section 4.1 states, "The meta-requirements relate to one or more design principles" and Fig. 3 "Formulation and categorization of meta-requirements and design principles" (article p. 5) draws the 45 mappings. The connector endpoints were recovered from the PDF vector paths; the shared Signaling box contains DP1 and DP2 and is expanded to both. Table 2 "Design principle overview" (article p. 6) formulates DP1-DP6; Fig. 6 (article p. 8) maps 14 design features to them.
